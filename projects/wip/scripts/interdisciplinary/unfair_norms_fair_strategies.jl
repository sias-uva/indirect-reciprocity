...
filters = [:norm => ByRow(!is_fair), [:majority_strat, :minority_strat] .=> ByRow(is_fair)]
# filters = []
...
df = find_ESS(p; mutant_filter=is_fair)

# begin
#     df_fair_invaders = find_ESS(p, mutant_filter=x->is_fair(x))
#     df_unfair_invaders = find_ESS(p, mutant_filter=x->!is_fair(x))
#     rename!(df_fair_invaders, :is_ess => :is_ess_fair)
#     rename!(df_unfair_invaders, :is_ess => :is_ess_unfair)
#     df_join = outerjoin(
#         df_fair_invaders, 
#         df_unfair_invaders, 
...
#     subset!(
#         df_join, 
#         [:is_ess_fair, :is_ess_unfair] => ByRow((x, y) -> x != y)
#     )
#     subset!(df_join, :is_ess_fair, [:majority_strat, :minority_strat] .=> ByRow(is_fair))
#     df_join.invader = map(eachrow(df_join)) do row
#         group, s, ms, _... = invader(get_agents(row.norm, row.majority_strat, row.minority_strat; p)..., p.prop_maj, p.utilities)
#         evalpoly(2, reshape(ms, Size(4))) 
#     end
#     df = df_join
#     rename!(df, :is_ess_fair => :is_ess)
# end
...
let
    ...
    # fig[0, :] = Label(fig,
    #     "Fairness and cooperation are achievable given the right combination of social norm and strategies.",
    #     fontsize=24, word_wrap=true, width=resolution[1] - 100, justification=:left, font=:bold
    # )
...
let
    ...
    display(
        subset(
            quadrant_df,
            :is_ess,
            :label => ByRow(==("Discriminates on relation")),
...
let
    ...
    # Norm colours by what do they discriminate vs (none, one , other, both),
    df.discrimination = map(eachrow(df)) do row
        n = iNorm(row.norm)
        # Does the norm discriminate based on
        disc_rel = n[1, :, :] != n[2, :, :] # Recipient group relation
        disc_rep = n[:, 1, :] != n[:, 2, :] # Recipient reputation
    ...
    df.label = map(eachrow(df)) do row
        "$(discrimination_dict[row.discrimination])"
    end
end
