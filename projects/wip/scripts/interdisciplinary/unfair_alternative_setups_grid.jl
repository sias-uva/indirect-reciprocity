...
player_perception_mistake_rate = SA[0.00, 0.0]
judge_perception_mistake_rate = SA[0.00, 0.00, 0.0]
...
# Functions currently rely on the globals above oops. Oh well they're only meant
# for this script.

function get_df_vector(ps, varname; index=1)
    return map(ps) do p
        df = rename!(
        ...
        df[!, varname] .= p[varname][index]
        df
    end
end

function lineplot_cf(fig_or_scene, res, varname; axis_kwargs)
    ax = Axis(fig_or_scene; axis_kwargs...)
    for (i, norm_int) in enumerate(norm_ints)
        data = subset(res, :norm => ByRow(==(norm_int)))
        always_ess, ess_cutoff_candidate = findmin(data.is_ess)
        ess_cutoff = !always_ess ? ess_cutoff_candidate : nrow(data)
        # @show data[1:ess_cutoff, varname]
        lines!(
            ax,
            data[1:ess_cutoff, varname],
...
let
    ...
    begin
        ...
        pem_ps = (merge(p, (; min_em=rate)) for rate in property_range)
    ...
    psave(fig, "unfair_alternative_setups_grid"; project="interdisciplinary")
    # for filetype in ("png", "pdf")
    #     save("figures/interdisciplinary/unfair_alternative_setups_grid.$filetype", fig)
    # end
    fig
end
