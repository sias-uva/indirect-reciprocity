...
begin
    ...
    linesegments!(
        [(Point2(0.75, 0.75), Point2(0.7, 0.2)), (Point2(0.69, 0.84), Point2(0.15, 0.8))];
        color=hiroshige[4],
        linewidth=2,
        linestyle=:dash,
    )
    text!(0.57, 0.94; text=rich("Consistent"; color=hiroshige[1], font=:bold), rotation=0)
    text!(
        0.76, 0.5; text=rich("Parochial"; color=hiroshige[4], font=:bold), rotation=-π / 40
    )
    text!(
        0.3,
        0.83;
    ...
    text!(0.36, 0.46; text=rich("Suboptimal"; color=hiroshige[5], font=:bold), rotation=0)
    markersize = 11
    ...
    for filetype in ("pdf", "png")
        save("projects/aamas/figures/coop_fairness_rl_scatter_poster.$filetype", fig)
    end
    fig
end
