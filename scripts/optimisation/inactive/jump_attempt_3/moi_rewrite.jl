...
red_mutant_strategies = SVector{15, eltype(all_strategies)}(setdiff(all_strategies, [parameters.red_strategy]))
blue_mutant_strategies = SVector{15, eltype(all_strategies)}(setdiff(all_strategies, [parameters.blue_strategy]))
...
function jump_incumbent_payoffs(jem, jpm1::T1, jpm2::T1, jpm3::T1, rem, rpm1::T2, rpm2::T2, bem, bpm1::T3, bpm2::T3, prop_red, ut1::T4, ut2::T4, ut3::T4, ut4::T4; p) where {T1<:Real,T2<:Real,T3<:Real,T4<:Real}
    ...
    # jpm = SVector{3, T1}(jpm1, jpm2, jpm3)
    # judge = Agent(p.norm, jem, jpm)
    red_rep = memoized_incumbent_reps[1](jem, jpm1::T1, jpm2::T1, jpm3::T1, rem, rpm1::T2, rpm2::T2, bem, bpm1::T3, bpm2::T3, prop_red)
    blue_rep = memoized_incumbent_reps[2](jem, jpm1::T1, jpm2::T1, jpm3::T1, rem, rpm1::T2, rpm2::T2, bem, bpm1::T3, bpm2::T3, prop_red)
...
function jump_ess_constraints(jem, jpm1::T1, jpm2::T1, jpm3::T1, rem, rpm1::T2, rpm2::T2, bem, bpm1::T3, bpm2::T3, prop_red, ut1::T4, ut2::T4, ut3::T4, ut4::T4; p) where {T1<:Real,T2<:Real,T3<:Real,T4<:Real}
    jpm = SVector{3,T1}(jpm1, jpm2, jpm3)
    rpm = SVector{2,T2}(rpm1, rpm2)
    bpm = SVector{2,T3}(bpm1, bpm2)
    utilities = SVector{4,T4}(ut1, ut2, ut3, ut4)
...
function construct_model(p)
    ...
    xs = judge_em, judge_pm..., red_em, red_pm..., blue_em, blue_pm..., prop_red, utilities...
    for i in 0:15
        ...
        @eval begin
            ...
            register($model, Symbol(string("red_ess_", $i)), 15, $(red_name); autodiff=true)
        ...
        @eval begin
            ...
            register($model, Symbol(string("blue_ess_", $i)), 15, $(blue_name); autodiff=true)
...
a = jump_ess_constraints(0.01, 0.00, 0.00, 0.00, 0.01, 0.00, 0.00, 0.01, 0.00, 0.00, 0.9, 2,2,1,1;p=parameters)

red_ess_12(0.01, 0.00, 0.00, 0.00, 0.01, 0.00, 0.00, 0.01, 0.00, 0.00, 0.9, 2.0,2.0,1.0,1.0)
